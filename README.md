...
A collection of Helm deployment scripts for OpenEBS LVM LocalPV, supporting online and offline installation, upgrades, uninstallation, and offline media packaging. These scripts are derived from the `deploy/scripts` directory of the original repository and have been extracted separately to facilitate usage in offline environments or with self-hosted image registries.
...
- Online/Offline Installation and Upgrades: Automatically fetches remote resources or utilizes local charts and images to complete deployment.
- Offline Media Packaging: One-click download of Helm charts and images to generate a portable offline package.
- Image Import and Push: Supports Docker and Podman; automatically retags and pushes images to a private registry when necessary.
- Resource and Node Selection: Configures node labels, resource quotas, namespaces, and other parameters via environment variables.
- Idempotent Operations and Logging: Scripts are idempotent, and logs are output to `/tmp/openebs-lvmlocalpv_*.log` for easy troubleshooting.
